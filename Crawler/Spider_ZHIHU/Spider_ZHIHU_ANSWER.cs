...
namespace Spider_ZHIHU
{
    public class Spider_ZHIHU_ANSWER:IController
    {
        ...
        public Spider_ZHIHU_ANSWER(String key, int max = 5,int answerCount=5)
        {
            this.key = key;
            this.max = max;
            this.answerCount = answerCount;
            searchSpider = new SearchSpider_ZHIHU(max);
        ...
        public async Task<bool> StartCrawling()
        {
            return await Task.Run(() =>
            {
                ...
                {
                    //爬取5个问题
                    if (getAnswerSpider.Count > max-1)
                    {
                        break;
                    }
                    ...
                    {
                        getAnswerSpider.GetFunc(new Uri("https://www.zhihu.com/api/v4/questions/"+url+"/answers?include=data%5B%2A%5D.is_normal%2Cadmin_closed_comment%2Creward_info%2Cis_collapsed%2Cannotation_action%2Cannotation_detail%2Ccollapse_reason%2Cis_sticky%2Ccollapsed_by%2Csuggest_edit%2Ccomment_count%2Ccan_comment%2Ccontent%2Ceditable_content%2Cvoteup_count%2Creshipment_settings%2Ccomment_permission%2Ccreated_time%2Cupdated_time%2Creview_info%2Crelevant_info%2Cquestion%2Cexcerpt%2Crelationship.is_authorized%2Cis_author%2Cvoting%2Cis_thanked%2Cis_nothelp%2Cis_labeled%3Bdata%5B%2A%5D.mark_infos%5B%2A%5D.url%3Bdata%5B%2A%5D.author.follower_count%2Cbadge%5B%2A%5D.topics&limit="+ answerCount +"&offset=0&platform=desktfop&sort_by=default")).Wait();
                    }
                }
                return true;
            });
    ...
    public class SearchSpider_ZHIHU:HTSpider
    {
        ...
        public SearchSpider_ZHIHU(int max)
        {
            this.max = max;
